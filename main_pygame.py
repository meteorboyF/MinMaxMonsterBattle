...
try:
    ...
    monster_sprites = {
        'Squirtle': pygame.image.load(os.path.join(assets_path, 'squirtle.png')).convert_alpha(),
...
def get_sprite(name):
    return pygame.transform.scale(monster_sprites.get(name), (250, 250))
...
def draw_text(text, font, color, surface, rect, aa=False, center=False):
    if center:
        ...
        y = rect.top
        for line in lines:
            line_surf = font.render(line, aa, color)
            surface.blit(line_surf, (rect.left, y))
            y += font.get_height()

def draw_health_bar(monster, x, y):
    bar_rect = pygame.Rect(x, y, 200, 25)
    ratio = max(0, monster.display_hp / monster.max_hp)
    ...
    if ratio > 0:
        pygame.draw.rect(screen, health_color, (x, y, bar_rect.width * ratio, bar_rect.height))
    hp_text = f"{int(monster.display_hp)}/{monster.max_hp}"
    hp_surf = small_font.render(hp_text, True, WHITE)
    draw_text("HP", small_font, WHITE, screen, bar_rect.inflate(-10, -10))
    screen.blit(hp_surf, (bar_rect.right - hp_surf.get_width() - 5, bar_rect.y + 2))
    
def draw_ui(player_mon, ai_mon):
    # AI Monster UI
    pygame.draw.rect(screen, GRAY, (20, 20, 300, 110), border_radius=10)
    pygame.draw.rect(screen, BLACK, (20, 20, 300, 110), 3, 10)
    draw_text(ai_mon.name, main_font, WHITE, screen, pygame.Rect(40, 30, 280, 110))
    draw_health_bar(ai_mon, 40, 70) 
    if ai_mon.status != 'ok':
        status_surf = small_font.render(ai_mon.status.upper(), True, STATUS_COLORS[ai_mon.status])
        screen.blit(status_surf, (325 - status_surf.get_width(), 35))
    
    # Player Monster UI
    pygame.draw.rect(screen, GRAY, (480, 340, 300, 110), border_radius=10)
    pygame.draw.rect(screen, BLACK, (480, 340, 300, 110), 3, 10)
    draw_text(player_mon.name, main_font, WHITE, screen, pygame.Rect(500, 350, 280, 110))
    draw_health_bar(player_mon, 500, 390) 
    if player_mon.status != 'ok':
        status_surf = small_font.render(player_mon.status.upper(), True, STATUS_COLORS[player_mon.status])
        screen.blit(status_surf, (785 - status_surf.get_width(), 355))
...
def draw_bottom_panel(message, mouse_pos, monster_list, player_mon, mode='message'):
    ...
    if mode == 'message':
        ...
        for i, move in enumerate(player_mon.moves):
            rect = pygame.Rect(420 + (i % 2) * 180, 470 + (i // 2) * 55, 170, 50)
            buttons.append(rect)
            ...
            pp_surf = small_font.render(pp_text, True, WHITE)
        ...
        for i, mon in enumerate(monster_list):
            rect = pygame.Rect(30 + (i % 4) * 190, 520, 180, 60)
            switch_buttons.append((rect, mon))
...
def draw_menu(title, options, mouse_pos):
    buttons = []
    draw_text(title, pygame.font.Font(None, 72), WHITE, screen, screen.get_rect(center=(SCREEN_WIDTH // 2, 150)), center=True)
    for i, option in enumerate(options):
        rect = pygame.Rect(SCREEN_WIDTH/2 - 150, 250 + i * 80, 300, 60)
        buttons.append(rect)
        color = BUTTON_HOVER_COLOR if rect.collidepoint(mouse_pos) else BUTTON_COLOR
        pygame.draw.rect(screen, color, rect, border_radius=10)
        draw_text(option, main_font, WHITE, screen, rect, center=True)
    return buttons
...
class Animator:
    def __init__(self, is_ai):
        self.monster = None
        self.is_ai = is_ai
        self.base_pos = (50, 240) if not is_ai else (500, 30)
        self.offset = [0, 0]
    ...
    def update(self):
        ...
        if elapsed > self.animation_duration:
            ...
            self.flash_color = (255, 255, 255, 0)
            return
        if self.animation_duration == 0.5: # Attack animation
            prog = elapsed / self.animation_duration
            self.offset[0] = 50 * (prog * 2 if prog < 0.5 else (1 - prog) * 2) * (-1 if self.is_ai else 1)
        if self.animation_duration == 0.3: # Damage animation
            prog = 1 - abs(0.5 - elapsed / self.animation_duration) * 2
            self.flash_color = (255, 255, 255, 180 * prog)
    
    def draw(self, surface, screen_offset):
        if not self.monster: return
        pos = (self.base_pos[0] + self.offset[0] + screen_offset[0], self.base_pos[1] + self.offset[1] + screen_offset[1])
        sprite = get_sprite(self.monster.name)
        if self.is_ai:
            sprite = pygame.transform.flip(sprite, True, False)
        if self.flash_color[3] > 0:
            flash_surf = pygame.Surface(sprite.get_size(), pygame.SRCALPHA)
            flash_surf.fill(self.flash_color)
            sprite.blit(flash_surf, (0, 0))
        surface.blit(sprite, pos)

# --- Main Game Loop ---
def main():
    gm, battle, vfx = GameManager(), Battle(), VFXManager()
    state = 'MAIN_MENU'
    msg = ""
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and not p_anim.is_active and not ai_anim.is_active and time.time() > anim_timer:
                if state == 'MAIN_MENU':
                    if buttons and buttons[0].collidepoint(mouse_pos):
                        state = 'CHOOSE_STARTER'
                elif state == 'BATTLE_WON':
                    state = 'PRE_BATTLE_SWITCH'
                    msg = "Choose your lead Pokémon!"
                    ...
                    for rect, mon in s_btns:
                        if rect.collidepoint(mouse_pos):
                            ...
                            if gm.set_next_opponent():
                                state = 'BATTLE_INTRO'
                                msg = f"Next opponent: {gm.ai_mon.name}!"
                            else:
                                state = 'GAME_OVER'
                    ...
                    for rect, mon in s_btns:
                        if rect.collidepoint(mouse_pos) and not mon.is_fainted:
                            if state == 'CHOOSE_STARTER':
                                gm.set_player_starter(mon.name)
                            else:
                                gm.player_mon = mon
                            p_anim.set_monster(gm.player_mon)
        ...
        if state == 'MAIN_MENU':
            buttons = draw_menu("Minimax Monster Battle", ["Start Game"], mouse_pos)
        else:
            if gm.ai_mon: ai_anim.draw(screen, render_offset)
            if gm.player_mon:
                p_anim.draw(screen, render_offset)
                draw_ui(gm.player_mon, gm.ai_mon)
            ...
            if state == 'AWAITING_INPUT':
                ...
                m_btns, s_btns = draw_bottom_panel(msg, mouse_pos, [], None, mode='message')
        
        vfx.draw(screen)
        pygame.display.flip()

        # State Machine Logic
        if time.time() < anim_timer or p_anim.is_active or ai_anim.is_active:
            continue
        ...
        if state == 'BATTLE_INTRO':
            ...
            if action == 'PLAYER_TURN':
                ...
                if gm.player_mon.status == 'paralyzed' and random.random() < 0.25:
                    msg = f"{gm.player_mon.name} is paralyzed!"
                    vfx.add_effect("Paralyzed!", STATUS_COLORS['paralyzed'])
                else:
                    move.current_pp -= 1
                    msg = f"{gm.player_mon.name} uses {move.name}!"
                    p_anim.start_attack()
                    if move.power > 0: 
                        info = battle.calculate_damage(gm.player_mon, gm.ai_mon, move)
                        gm.ai_mon.take_damage(info['damage'])
                        ai_anim.start_damage()
                ...
                if mon.is_fainted:
                    turn_q.clear()
                    msg = f"{mon.name} fainted!"
                    anim_timer = time.time() + MESSAGE_DURATION
                    if mon is gm.ai_mon:
                        gm.add_defeated_to_roster(mon)
                        gm.full_heal_player_team()
                        state = 'BATTLE_WON'
                    elif any(not m.is_fainted for m in gm.player_team):
                        state = 'PLAYER_FORCED_SWITCH'
                    else:
                        state = 'GAME_OVER'
                        msg = "Your team has fainted. Game Over."
            
            elif action == 'END_TURN':
                if not any(m.is_fainted for m in [gm.player_mon, gm.ai_mon]):
                    state = 'AWAITING_INPUT'
